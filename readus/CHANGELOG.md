...
### AI 服务商配置优化
- 账号设置中 AI 服务商表单新增右侧「如何填写」帮助面板，含各平台 Base URL 示例表格
- Base URL 保存时自动补齐 `/v1` 路径（GPT/DeepSeek/Ollama/哈基米等 OpenAI 兼容服务商）
- Azure 路径（含 `/openai/deployments`）和 Gemini 不做自动补齐
- 模型列表输入框下方增加提示："不填则使用自动获取"
- AI 分析接口错误信息中文化：区分连接失败/超时/404，给出针对性排查建议
- 数据库文档更新 `ai_providers` 表说明和 FAQ
