...
- **题块标题改不动（第 2 条）**：`autoNameBlocks` 在每次编辑、载入、保存时无条件重算标题，输入框内容被立即覆盖。改为只接管「空标题 / 新建默认名 / 上一轮自动生成标题」，人工标题保留（`cardModel.isAutoBlockTitle`）。
- **解答题横线上方加字与加粗/斜体（第 3 条）**：注记（`subjective_questions.annotation`）此前只在填空题块渲染，现除作文块外的主观题都可填写，并支持 `**加粗**` / `*斜体*`；排版、SVG 预览、PDF 导出共用 `shared/richText` 解析，不新增数据库列。
- **作文格跨栏与 PDF 题块重叠（第 4、5 条）**：作文格此前一次铺满三栏，100 字会排成三竖趟且横线压住其他题块。改为一行只占一栏宽（A3 纸宽三等分），填满一栏再换下一栏 / 下一页；预览与导出 PDF 共用同一几何，同步修正。格子高度超过版心时改为给出排版警告，不再反复新建空页。
- **客观题选项间距（第 6 条）**：排布保持原样（A3 一行 3 题、A4 一行 4 题），仅在题目实际可用宽度装不下时才收紧选项间距，避免选项压到相邻题目（4 选项题目间距与既有卡片完全一致）。
- **教师权限下拉为空（第 7 条）**：`/api/teachers` 返回 `teachers`，前端读 `users`，教师下拉恒为空；修正后列出全部教师账号（不再按 `role_name` 过滤）。
- **深色模式散点图（第 10 条）**：难度-区分度诊断的档位色在暗底上不可辨，深色模式下数据点与疑题描边改用主题危险色（红），亮色模式保持按档位着色。

验证：`npm run typecheck`、`npm run verify:feedback-260923`（新增：题块标题可改名 / 作文格单栏换行 / 客观题一行 4 题 / 解答题注记）、`npm run verify:core-logic`。
未处理：第 1 条（预览窗口内拖动题块，反馈允许与自动分栏冲突时暂缓）、第 8 条（班级改名 + 班主任 / 分科任课教师设置，待单独一轮）。
...
- **教师关联班级首次不显示**：选中教师时自动请求详情，分离列表与详情状态，取消过期请求；明确加载和失败提示，刷新及关联变更后同步当前详情，避免必须手动刷新才能看到班级。
- **绑定考试的班级无法删除**：将班级和年级删除改为归档，从当前管理列表隐藏并保留历史考试、成绩和关系记录；归档前固化相关应考名单，事务失败回滚，新考试禁止绑定归档范围。新增 SQLite / MariaDB 迁移 `50 / archive-class-and-grade`，同步初始化 schema。

教师及归档代码已包含在 `e25b769`，本轮未部署到生产。验证包含教师页面浏览器交互（隔离 HTTP 夹具）、SQLite 与真实 MariaDB 归档回归、类型检查、相关权限与安全回归，以及 Web、扫描端完整构建。归档修复尚不包含分班、升年级所需的完整历史班级快照，部分历史统计仍受当前班级关系影响。
...
### 1. 检测失败不再阻止扫描（「扫描端阻止我扫」的直接原因）
- 根因：`ScannerPanel` 的「扫描设置」整卡（含图片去向档位与「开始扫描」按钮）被 `sources.length > 0` 门控，且 `detectSources()` 任何非 ok 都置 `error` 态——**扫描仪一旦检测失败，界面连扫描入口都不存在**，而设备本身可用。这与「DLL 正常、能扫码」的现场实测完全吻合。
- 修复：设置卡与「开始扫描」在 `ready / idle / error` 三态**始终渲染**；数据源字段在无枚举结果时退化为**可手工输入**（留空 = 交由桥接请求系统默认数据源 `MSG_GETDEFAULT`），并附「重新检测」；`startScan()` 不再因「未选到数据源」静默 return（服务端 `sourceName` 本就缺省为 `""`）。
- 移除「点击上方按钮检测扫描仪」独立提示块（与新设置卡内入口重复，易误导）。

### 2. 「图片去向」提升为工作台常驻首位
- 此前直扫档位藏在扫描面板的「扫描设置」里（见第 1 条，检测失败即消失），导入档位在「导入阅卷」卡内，两处入口深浅不一、老师找不到。
- 现固定在扫描工作台**左侧首位**新增「图片去向」卡（仅本地 / 本地判分+上传服务器），直扫与导入共用同一 `useScannerMode` 记忆与实时同步；未配置可用服务器时给出明确警示与**「去配置」**按钮（`ServerConfigDialog` 移出皮肤条件块，保证任何情况下都能打开）。
- 「导入阅卷」卡内的重复档位控件改为**只读回显**（同一栏相邻两个同名控件会让人分不清哪个生效）；扫描面板内保留同源镜像控件并统一标题为「图片去向」。

### 3. 服务器地址归一化与校验（「扫描端→主站链路受阻」的根因）
- 根因：服务器地址全程只做 `trim()` + 去尾斜杠，**从不补协议头**。教师按习惯填 `192.168.1.100:5174`（省略 `http://`）时，`fetch("192.168.1.100:5174/api/...")` 的 scheme 以数字开头不是合法 URL，浏览器直接抛 TypeError——**请求根本没发出去，服务端日志零痕迹**；而 `isRemoteServerConfigured()` 只判非空，界面仍显示"已配置"，于是上传永远失败且无从排查。
- 修复：新增 `normalizeServerUrl()` / `isValidServerUrl()` / `readServerUrl()` / `writeServerUrl()`，**四个读取点全部收敛**（`auth/api.ts` 的 `getApiBase`/`getRemoteScannerBase`、`remoteServerStatus.ts` 探活、`scannerUploadManager.ts` 任务快照 base）；配置对话框保存前校验并**回显实际生效地址**，输入缺协议头时实时提示"将按 http://… 使用"。
- `isRemoteServerConfigured()` 改为**要求归一化后是合法 http(s) 地址**，杜绝「填了但发不出去」被判为已配置而静默失败。
- `测试连接`/上传失败文案带上具体 base 与 HTTP 状态；「未启用远程扫描客户端 API」明确指向 `PROJECTX_ENABLE_SCANNER_CLIENT_API=1`。

### 4. 诊断可一键复制 · 错误文案校正
- 检测失败处新增**「复制诊断」**按钮（含时间戳、答题卡 ID、错误信息与全部技术细节；剪贴板不可用时回退隐藏 textarea），老师现场无法翻日志也可直接粘贴反馈。
- `describeBridgeFailure` 的 `0xC0000135` 文案**不再一口断定"缺少 VC++ 运行库"**：v2.5.5 起运行库已随包分发，现场实测 DLL 齐全却仍报此码，故改为并列可能原因（安装不完整 / 被安全软件隔离）并明确「该错误由 Windows 报告，不代表扫描仪或驱动有问题」。
- 文档：`readus/SCANNER-SETUP.md` 更新流程说明与 FAQ（检测失败不阻止扫描、图片去向位置、上传不通三步排查、0xC0000135 新解释），版本号统一为 v2.5.5+。
...
### 弱网韧性（v2.5.1）
- 单页失败自动重试 2 次（退避 1s/3s，同 token 服务端幂等覆盖）；单页超时 120s。
- 断线暂停/恢复：结合 health 探活与 online 事件，断线置 paused 提示，网络恢复自动续传；存在失败页时不发 complete，服务端会话保持可续传态。
- 彻底失败保留任务并列出失败页码，支持手动「重试失败页」（已成功页不重传）。

### 进度与服务状态可视化（v2.5.1）
- 新增全局右上角下弹上传进度卡（挂 ScannerApp 根部，仅扫描端构建包含）：排队/进度条 x/y 页/已断线/失败明细/完成 3 秒自动收起。
- 新增 `remoteServerStatus.ts` 周期探活远端 `/api/app/health`（正常 20s、异常加速 5s），区分 未配置/在线/离线/扫描API未启用 四态；工作台顶栏常驻状态指示器，探活结果反哺上传暂停判定。

### 其他
- 服务端零改动（协议、token 幂等、鉴权均沿用既有实现）；Web 教师端构建不受影响。
- 设计文档：`docs/superpowers/specs/2026-08-25-scanner-upload-manager-design.md`。
