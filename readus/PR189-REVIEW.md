...
> 与 `origin/main`（`#187`）相比仅 1 个提交、10 个文件、+140/−14。
...
1. **前端未消费 `scoringMode`**：当前 GradePanel 硬编码走题块总分。若要真正让「逐题」配置生效，前端需读取题块配置并切换输入形态（否则 B2 的校验只在误用时触发，正常流仍由 GradePanel 决定）。建议在 `GradePanel` 接入 `config.scoringMode` 判断。
