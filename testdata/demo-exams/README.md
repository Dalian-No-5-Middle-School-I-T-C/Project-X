...
覆盖 Project-X **v1.9.4 ~ v2.2.9**（含答题卡设计器修复：作文格仿高考样式 #830c6a7、客观题选项竖排、填空题图片/注释回归）功能测试所需的完整数据集（文理分科 #212、填空题升级 #211、网阅打分记录/双评仲裁/断点续批 #173/#177、成绩公布 PR #256）。
...
- **作文格演示**：演示-语文卡（88000001）含 essay 块 1 道题（仿高考样式：朱红格线 / 每 100 字刻度 / 粗边框 / 行间虚线）
- **选项竖排演示**：演示-数学卡（88000002）客观题块 `option_layout=vertical-options`（A/B/C/D 纵向堆叠）
...
- **作文格仿高考样式**：88000001 卡 essay 块 — 目标 600 字、朱红格线 `#c00000`（ESSAY_DEFAULT_LINE_COLOR）、每 100 字刻度（跨页续号）、粗边框 + 行间虚线、题号右对齐；行数/行缝几何由 `src/shared/essayGrid.ts` 唯一事实源推导（预览/PDF/排版引擎三端一致）
- **客观题选项竖排**：88000002 卡客观题块 `option_layout=vertical-options`（题号下方 A/B/C/D 纵向堆叠），行高按 span 预留、识别坐标自动跟随；服务端 `normalizeOptionLayout` 白名单持久化验证（保存后不降级为 horizontal）
- **网阅打分**（演示-网阅测试）：
...
- 数据库迁移版本：v1~v45（v17、v39/v40 跳过/作废；v41/v42=成绩公布 PR #256，v43~v45=控制台/权限五维约束），所有迁移幂等（`addColumnIfMissing` / `CREATE TABLE IF NOT EXISTS`）
...
- ZIP 恢复后必须重启 dev 服务，`initializeDatabase()` 会自动执行缺失迁移（v25~v45 等）
