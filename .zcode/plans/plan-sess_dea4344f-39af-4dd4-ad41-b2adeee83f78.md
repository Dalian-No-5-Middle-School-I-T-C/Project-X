# 成绩分析界面优化重构 + 选项分析 + 跨班对比 + 阈值体系 — 实施计划 v2

> 已结合《成绩分析能力差距分析与建设路线图》更新：并入 **P0-1 可配置阈值** 与 **P0-4 轻量版（薄弱项分层+预警）**；本文档的 P2-1（作答痕迹/选项回放）即本计划的选项分析功能。
...
| 路线图并入 | ✅ P0-1 可配置阈值（全局级）、✅ P0-4 轻量版分层+预警；❌ P0-2 任意两场对比、P0-3 标准分归一、P2-3 分数段占比趋势、全部 P1 大项 → 后续迭代 |
| 学生端 | 本次只做教师端 |
...
1. 设计文档落盘 `docs/superpowers/specs/2026-07-29-grade-analysis-redesign-design.md` 并提交（计划模式禁写，顺延至此）。
