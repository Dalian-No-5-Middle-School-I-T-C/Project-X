...
export function StudentAreaSvg({ area }: { area: NonNullable<LayoutDocument["pages"][number]["studentArea"]> }) {
  const notesLines = area.notesText?.split("\n").filter((line) => line.trim().length > 0) ?? [];
  const hasCombined = area.combinedRect !== undefined;
  return (
    <g>
      {area.boxInfo !== false && (
      ...
      {area.combinedRect && (
        <rect {...area.combinedRect} fill="none" stroke="#333" strokeWidth="0.25" />
      )}
      {area.fieldRows.map((row, index) => (
        <g key={index}>
          <text x={row.labelX} y={row.labelY} className="svg-label">
            {row.label}
          </text>
      ...
      {(() => {
        const notesRect = area.notesRect;
        if (!notesRect) return null;
        return (
          <>
            {!hasCombined && <rect {...notesRect} fill="none" stroke="#333" strokeWidth="0.25" />}
            <text x={notesRect.x + notesRect.width / 2} y={notesRect.y + 4.7} textAnchor="middle" className="svg-label">
              注意事项
            </text>
            <line x1={notesRect.x} y1={notesRect.y + 5.5} x2={notesRect.x + notesRect.width} y2={notesRect.y + 5.5} stroke="#333" strokeWidth="0.2" />
            {notesLines.map((line, index) => (
              <text key={index} x={notesRect.x + 3} y={notesRect.y + 9 + index * 4.8} className="svg-notes">
                {line.trim()}
              </text>
            ))}
          </>
      ...
      {area.combinedRect && area.notesRect && area.digitRect.width > 0 && (() => {
        const sepX = area.notesRect.x + area.notesRect.width + (area.digitRect.x - (area.notesRect.x + area.notesRect.width)) / 2;
        return <line x1={sepX} y1={area.combinedRect.y + 4} x2={sepX} y2={area.combinedRect.y + area.combinedRect.height - 4} stroke="#333" strokeWidth="0.2" />;
      })()}
      {area.digitRect.width > 0 && (
        <>
          {!hasCombined && <rect {...area.digitRect} fill="none" stroke="#333" strokeWidth="0.25" />}
          <text x={area.digitRect.x + area.digitRect.width / 2} y={area.digitRect.y + 5.2} textAnchor="middle" className="svg-label">
            填涂号区
          </text>
          {/* 左侧空框列：供考生手填考号位 */}
          {(area.digitLabelCells ?? []).map((label) => (
            <rect key={`lbl_${label.digitIndex}`} {...label.rect} fill="#fff" stroke="#333" strokeWidth="0.15" style={{ fill: "#fff" }} />
          ))}
          {/* 顶部表头行：0-9 列标 */}
          {(area.digitHeaderCells ?? []).map((header) => (
            <g key={`hdr_${header.digit}`}>
              <rect {...header.rect} fill="#fff" stroke="#333" strokeWidth="0.15" style={{ fill: "#fff" }} />
              <text x={header.rect.x + header.rect.width / 2} y={header.rect.y + header.rect.height / 2} textAnchor="middle" dominantBaseline="middle" className="svg-tiny">
                {header.digit}
              </text>
            </g>
          ))}
