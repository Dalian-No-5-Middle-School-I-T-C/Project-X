...
export async function seedFillBlankDemo(db: DbAdapter): Promise<void> {
  ...
  await db.run(
    ...
    blockId, FILL_BLANK_CARD_ID, 0, "fill_blank", "填空题（演示）"
  );
  ...
  const insertQ = buildInsertIgnore(db.dialect, "subjective_questions", [
    ...
    "blanks_count", "blanks_width_mm", "blanks_height_mm", "blanks_label_style",
    "blanks_items_json", "annotation", "sort_order"
  ]);
  const insertImg = buildInsertIgnore(db.dialect, "subjective_question_images", [
    "question_id", "asset_id", "original_name", "width_mm", "height_mm", "align", "sort_order"
  ]);

  // Q1：两空，逐空自定义横线宽度/高度 + 右侧批注 + 文字注释（blanks_items_json 与设计器存储格式一致）
  await db.run(
    insertQ,
    "fb-q1", blockId, 1, 6, "plain_subjective", "blank", 14, 2, 30, 6, "arabic_parentheses",
    JSON.stringify([
  ...
  await db.run(
    insertQ,
    "fb-q2", blockId, 2, 4, "plain_subjective", "blank", 14, 1, 30, 6, "arabic_parentheses",
    JSON.stringify([{ label: "(1)", widthMm: 30, heightMm: 6 }]),
    "观察下面的图片，先写出数量关系式，再列式解答。注意单位换算，结果保留一位小数。",
    1
  );
  await db.run(insertImg, "fb-q2", "fig-demo.png", "fig-demo.png", 48, 22, "center", 0);

  // Q3：一空普通横线（对照，仅文字注释）
  await db.run(
    insertQ,
    "fb-q3", blockId, 3, 3, "plain_subjective", "blank", 14, 1, 34, 6, "none",
    null,
    "注：答案不唯一，言之有理即可。",
    2
  );
