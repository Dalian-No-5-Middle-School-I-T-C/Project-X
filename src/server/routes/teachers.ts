...
router.get("/", async (req: Request, res: Response) => {
  ...
  const { teachers, total } = await userRepo.listTeachers({ keyword, page, pageSize });
...
router.get("/:id", async (req: Request, res: Response) => {
  ...
  const teacher = await userRepo.findTeacherById(id);
  if (!teacher) {
...
router.post("/:id/classes", async (req: Request, res: Response) => {
  try {
    ...
    const existing = await userRepo.findByIdIncludingInactive(teacherId);
    if (!existing || existing.role_id !== ROLE_IDS.TEACHER) {
      res.status(404).json({ message: "教师不存在" });
      return;
    }
    ...
    for (const classId of ids) {
      const cls = await classRepo.findClassById(classId);
      if (!cls) continue;
      await classRepo.addTeacherToClass(teacherId, classId, subject ? String(subject) : undefined);
      added++;
    }

    const teacher = await userRepo.findTeacherById(teacherId);
...
router.delete("/:id/classes/:classId", async (req: Request, res: Response) => {
  try {
    const teacherId = Number(req.params.id);
    const classId = Number(req.params.classId);
    await classRepo.removeTeacherFromClass(teacherId, classId);
...
router.get("/:id/classes", async (req: Request, res: Response) => {
  const teacherId = Number(req.params.id);
  res.json(await classRepo.listTeacherClasses(teacherId));
});
