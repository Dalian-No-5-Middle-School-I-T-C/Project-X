  ...
  try {
    ...
    if (sessions.length > 0) {
      ...
      const cropCount = await db.get(
        "SELECT COUNT(*) AS cnt FROM review_assignments WHERE exam_id = ? AND block_id = ? AND teacher_id = ?",
    ...
    if (latestExam) {
      ...
      const fallback = await db.get(
        `SELECT id, name, subject, created_at FROM exams ORDER BY created_at DESC LIMIT 1`
