...
- Open `http://127.0.0.1:5173/`. First DB initialization creates username `admin` with the fixed bootstrap password `admin123` (mirrored into `bootstrap-admin.txt` beside the database). No forced password change on first login; change it promptly in any network-exposed deployment.
